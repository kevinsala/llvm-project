...
// RUN: for i in $(seq 0 4); do %inputgen_gen $i > %inputgen_gen.$i.0.0.inp.gen.out; done
// RUN: for i in $(seq 0 4); do %inputgen_repl_gen %inputgen_gen.$i.0.0.inp $i > %inputgen_gen.$i.0.0.inp.repl.out; done
// RUN: for i in $(seq 0 4); do diff %inputgen_gen.$i.0.0.inp.gen.out %inputgen_gen.$i.0.0.inp.repl.out; done
