...
struct GenELF64KernelTy : public GenericKernelTy {
  ...
  Error initImpl(GenericDeviceTy &GenericDevice,
    ...
    if (auto Err = GHandler.getGlobalMetadataFromDevice(*this, Image, Global))
      return Err;

	// Check that the function pointer is valid.
    if (!Global.getPtr())
      Plugin::error("Invalid function for kernel %s", getName());
...
protected:
  ...
  Expected<OMPTgtExecModeFlags> getExecutionModeForKernel(
      StringRef Name, DeviceImageTy &Image) override {
